...
class TD3_BC_Penalty(object):
	...
	def train(self, replay_buffer, batch_size=100, main_timestep=0):
		...
		with torch.no_grad():
			...
			target_Q = reward + not_done * self.discount * (target_Q - self.alpha * F.mse_loss(policy_action, action))
		...
		if self.total_it > self.start_update_policy and self.total_it % self.policy_freq == 0:
			policy_action = self.actor(state)
			# Compute actor loss
