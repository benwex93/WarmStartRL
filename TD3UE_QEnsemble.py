...
class TD3UE_QEnsemble(object):
	def __init__(
		...
		# self.N_ensembles = 10
		self.N_copies_for_uncertainty = 50
		...
		self.critic_ensemble = [Critic(state_dim, action_dim).to(device) 
										for x in range(self.N_copies_for_uncertainty)]
		self.critic_ensemble_target = [copy.deepcopy(critic) for critic in self.critic_ensemble]
		self.critic_ensemble_optimizer = [torch.optim.Adam(critic.parameters(), lr=3e-4) \
																for critic in self.critic_ensemble]

		self.critic_ensemble.append(self.critic)
		self.critic_ensemble_target.append(self.critic_target)
		self.critic_ensemble_optimizer.append(self.critic_optimizer)
	...
	# 		Q_ensemble_value = [critic.Q1(state, action) for critic in self.critic_ensemble]
	...
	def train(self, replay_buffer, batch_size=100, main_timestep=0):
		...
		for critic, critic_target, critic_optimizer in zip(self.critic_ensemble, 
															self.critic_ensemble_target,
															self.critic_ensemble_optimizer):
